...
const (
	...
	orderStatusNewName                         = "New"
	orderStatusPaymentSystemCreateName         = "In payment"
	orderStatusPaymentSystemRejectOnCreateName = "Error"
	orderStatusPaymentSystemRejectName         = "Error"
	orderStatusPaymentSystemCompleteName       = "Paid"
	orderStatusProjectInProgressName           = "In progress"
	orderStatusProjectCompleteName             = "Completed"
	orderStatusProjectPendingName              = "Pending"
	orderStatusProjectRejectName               = "Reject"
	orderStatusRefundName                      = "Refund"
	orderStatusChargeBackName                  = "Chargeback"
	orderStatusPaymentSystemDeclinedName       = "Declined"
	orderStatusPaymentSystemCanceledName       = "Canceled"

	orderStatusNewDescription                         = "Project create new payment order in P1."
	orderStatusPaymentSystemCreateDescription         = "Payment order was create in payment system."
	orderStatusPaymentSystemRejectOnCreateDescription = "Payment system reject create order request."
	orderStatusPaymentSystemRejectDescription         = "Payment notification request from payment system was reject. Possible reasons: incorrect amount or currency."
	orderStatusPaymentSystemCompleteDescription       = "Payment notification request from payment system successfully complete."
	orderStatusProjectInProgressDescription           = "P1 started sending notification request about payment to project."
	orderStatusProjectCompleteDescription             = "Project successfully processed notification about payment from P1"
	orderStatusProjectPendingDescription              = "Project can't processed notification about payment from P1 and limit of notification attempts was ended."
	orderStatusProjectRejectDescription               = "Project reject notification about payment from P1. This payment is candidate to refund."
	orderStatusRefundDescription                      = "Payment was refunded to payer"
	orderStatusChargeBackDescription                  = "Customer's chargeback claim was received"
	orderStatusPaymentSystemDeclinedDescription       = "Payment system declined payment"
	orderStatusPaymentSystemCanceledDescription       = "User canceled payment on payment system side"

	bankCardFieldPanName            = "Card number"
	bankCardFieldHolderName         = "Card holder"
	eWalletFieldIdentifierName      = "Payment system account"
	cryptoFieldIdentifierName       = "Address"
	txnParamsFieldBankCardIs3DSName = "Is 3DS Secured"
	txnParamsFieldDeclineCodeName   = "Decline code"
	txnParamsFieldDeclineReasonName = "Decline reason"
	txnParamsFieldCryptoAmountName  = "Amount in crypto currency"
...
var orderStatusesNames = map[int]string{
	OrderStatusNew:                         orderStatusNewName,
	OrderStatusPaymentSystemCreate:         orderStatusPaymentSystemCreateName,
	OrderStatusPaymentSystemRejectOnCreate: orderStatusPaymentSystemRejectOnCreateName,
	OrderStatusPaymentSystemReject:         orderStatusPaymentSystemRejectName,
	OrderStatusPaymentSystemComplete:       orderStatusPaymentSystemCompleteName,
	OrderStatusProjectInProgress:           orderStatusProjectInProgressName,
	OrderStatusProjectComplete:             orderStatusProjectCompleteName,
	OrderStatusProjectPending:              orderStatusProjectPendingName,
	OrderStatusProjectReject:               orderStatusProjectRejectName,
	OrderStatusRefund:                      orderStatusRefundName,
	OrderStatusChargeBack:                  orderStatusChargeBackName,
	OrderStatusPaymentSystemDeclined:       orderStatusPaymentSystemDeclinedName,
	OrderStatusPaymentSystemCanceled:       orderStatusPaymentSystemCanceledName,
}

var orderStatusesDescription = map[int]string{
	OrderStatusNew:                         orderStatusNewDescription,
	OrderStatusPaymentSystemCreate:         orderStatusPaymentSystemCreateDescription,
	OrderStatusPaymentSystemRejectOnCreate: orderStatusPaymentSystemRejectOnCreateDescription,
	OrderStatusPaymentSystemReject:         orderStatusPaymentSystemRejectDescription,
	OrderStatusPaymentSystemComplete:       orderStatusPaymentSystemCompleteDescription,
	OrderStatusProjectInProgress:           orderStatusProjectInProgressDescription,
	OrderStatusProjectComplete:             orderStatusProjectCompleteDescription,
	OrderStatusProjectPending:              orderStatusProjectPendingDescription,
	OrderStatusProjectReject:               orderStatusProjectRejectDescription,
	OrderStatusRefund:                      orderStatusRefundDescription,
	OrderStatusChargeBack:                  orderStatusChargeBackDescription,
	OrderStatusPaymentSystemDeclined:       orderStatusPaymentSystemDeclinedDescription,
	OrderStatusPaymentSystemCanceled:       orderStatusPaymentSystemCanceledDescription,
}

var paymentRequisitesNames = map[string]string{
	pkg.PaymentCreateFieldPan:       bankCardFieldPanName,
	pkg.PaymentCreateFieldHolder:    bankCardFieldHolderName,
	pkg.PaymentCreateFieldEWallet:   eWalletFieldIdentifierName,
	pkg.PaymentCreateFieldCrypto:    cryptoFieldIdentifierName,
	pkg.TxnParamsFieldBankCardIs3DS: txnParamsFieldBankCardIs3DSName,
	pkg.TxnParamsFieldDeclineCode:   txnParamsFieldDeclineCodeName,
	pkg.TxnParamsFieldDeclineReason: txnParamsFieldDeclineReasonName,
	pkg.TxnParamsFieldCryptoAmount:  txnParamsFieldCryptoAmountName,
}
...
func (s *Service) FindAllOrders(
	ctx context.Context,
	req *grpc.FindAllOrdersRequest,
	rsp *billing.OrderPaginate,
) error {
	var f bson.M
	var pFilter []bson.ObjectId

	for k := range req.Projects {
		pFilter = append(pFilter, bson.ObjectIdHex(k))
	}

	filter := bson.M{"project.id": bson.M{"$in": pFilter}}

	if quickFilter, ok := req.Values[orderFilterFieldQuickFilter]; ok {
		r := bson.RegEx{Pattern: ".*" + quickFilter.Values[0].String() + ".*", Options: "i"}
		...
		f = s.processFilters(req.Values, filter)
	}

	co, err := s.db.Collection(pkg.CollectionOrder).Find(f).Count()
	...
	var o []*billing.Order
	if err := s.db.Collection(pkg.CollectionOrder).Find(f).Sort(req.Sort...).Limit(int(req.Limit)).Skip(int(req.Offset)).All(&o); err != nil {
		s.logError("Query from table ended with error", []interface{}{"table", pkg.CollectionOrder, "error", err})
		return err
	}

	var ot []*billing.OrderSimple

	if o != nil && len(o) > 0 {
		ot, err = s.transformOrders(o, req)
	...
	rsp = &billing.OrderPaginate{Count: int32(co), Items: ot}

	return nil
}

func (s *Service) processFilters(values map[string]*_struct.ListValue, filter bson.M) bson.M {
	if id, ok := values[orderFilterFieldId]; ok {
		if bson.IsObjectIdHex(id.Values[0].String()) {
			filter["_id"] = bson.ObjectIdHex(id.Values[0].String())
		} else {
			filter["_id"] = id.Values[0].String()
		}
	}

	if pms, ok := values[orderFilterFieldPaymentMethods]; ok {
		var fPms []bson.ObjectId

		for _, pm := range pms.Values {
			fPms = append(fPms, bson.ObjectIdHex(pm.String()))
		}

		filter["payment_method.id"] = bson.M{"$in": fPms}
	}

	if cs, ok := values[orderFilterFieldCountries]; ok {
		filter["payer_data.country_code_a2"] = bson.M{"$in": cs}
	}

	if ss, ok := values[orderFilterFieldStatuses]; ok {
		var ssi []int

		for _, s := range ss.Values {
			si, err := strconv.Atoi(s.String())
	...
	if a, ok := values[orderFilterFieldAccount]; ok {
		ar := bson.RegEx{Pattern: ".*" + a.Values[0].String() + ".*", Options: "i"}
		filter["$or"] = bson.M{"project_account": ar, "pm_account": ar, "payer_data.phone": ar, "payer_data.email": ar}
	}

	pmDates := make(bson.M)

	if pmDateFrom, ok := values[orderFilterFieldPMDateFrom]; ok {
		if ts, err := strconv.ParseInt(pmDateFrom.Values[0].String(), 10, 64); err == nil {
			pmDates["$gte"] = time.Unix(ts, 0)
		}
	}

	if pmDateTo, ok := values[orderFilterFieldPMDateTo]; ok {
		if ts, err := strconv.ParseInt(pmDateTo.Values[0].String(), 10, 64); err == nil {
	...
	if prjDateFrom, ok := values[orderFilterFieldProjectDateFrom]; ok {
		if ts, err := strconv.ParseInt(prjDateFrom.Values[0].String(), 10, 64); err == nil {
			prjDates["$gte"] = time.Unix(ts, 0)
		}
	}

	if prjDateTo, ok := values[orderFilterFieldProjectDateTo]; ok {
		if ts, err := strconv.ParseInt(prjDateTo.Values[0].String(), 10, 64); err == nil {
...
func (s *Service) transformOrders(orders []*billing.Order, params *grpc.FindAllOrdersRequest) ([]*billing.OrderSimple, error) {
	var tOrders []*billing.OrderSimple

	for _, oValue := range orders {
		tOrder := &billing.OrderSimple{
			Id: oValue.Id,
			Project: &billing.SimpleItem{
				Id:   oValue.Project.Id,
				Name: oValue.Project.Name[oValue.User.Locale],
			},
			Account:        oValue.ProjectAccount,
			ProjectOrderId: oValue.ProjectOrderId,
			PayerData: &billing.PayerData{
				Country: oValue.User.Address.Country,
				Zip:     oValue.User.Address.PostalCode,
				State:   oValue.User.Address.State,
			},
			ProjectAmountIncome: &billing.OrderSimpleAmountObject{
				Amount: oValue.ProjectIncomeAmount,
				Currency: &billing.SimpleCurrency{
			...
			Status: &billing.Status{
				...
				Name:        orderStatusesNames[int(oValue.Status)],
				Description: orderStatusesDescription[int(oValue.Status)],
			},
			VatAmount: oValue.Tax.Amount,
			CreatedAt: oValue.CreatedAt.Seconds,
		}

		if oValue.PaymentMethodIncomeAmount > 0 {
			tOrder.PaymentMethodAmountIncome = &billing.OrderSimpleAmountObject{
		...
		if oValue.AmountOutMerchantAccountingCurrency > 0 {
			tOrder.ProjectAmountOutcome = &billing.OrderSimpleAmountObject{
				Amount: oValue.AmountOutMerchantAccountingCurrency,
				Currency: &billing.SimpleCurrency{
					CodeInt: params.Merchant.Banking.Currency.CodeInt,
					CodeA3:  params.Merchant.Banking.Currency.CodeA3,
					Name:    params.Merchant.Banking.Currency.Name,
				},
			}
		}

		if oValue.PaymentMethod != nil {
			tOrder.PaymentMethod = &billing.SimpleItem{
			...
			tOrder.PaymentRequisites = s.preparePaymentRequisites(oValue)
		}

		if oValue.PaymentMethodOrderClosedAt != nil {
			tOrder.ConfirmedAt = oValue.PaymentMethodOrderClosedAt.Seconds
		}

		if oValue.ProjectLastRequestedAt != nil {
			tOrder.ClosedAt = oValue.ProjectLastRequestedAt.Seconds
		}
...
func (s *Service) preparePaymentRequisites(o *billing.Order) map[string]string {
	requisites := make(map[string]string)

	for k, v := range o.PaymentRequisites {
		n, ok := paymentRequisitesNames[k]
...
func (s *Service) GetRevenueDynamic(
	ctx context.Context,
	...
	var revPoints []*billing.RevenueDynamicPoint

	pRefund := res[0][revenueDynamicFacetFieldPointsRefund].([]interface{})
	pRevenue := res[0][revenueDynamicFacetFieldPointsRevenue].([]interface{})

	for _, v := range pRefund {
		vm := v.(map[string]interface{})

		vmId := vm[revenueDynamicFacetFieldId].(map[string]interface{})
		vmTotal := vm[revenueDynamicFacetFieldTotal].(float64)

		refPoints[getRevenueDynamicPointsKey(vmId).String()] = formatAmount(vmTotal)
	}
	...
	for _, v := range pRevenue {
		...
		revPointDate := getRevenueDynamicPointsKey(vmId)
		refVal, ok := refPoints[revPointDate.String()]

		revPoint := &billing.RevenueDynamicPoint{
			Date: revPointDate,
		}
	...
	rsp = &grpc.RevenueDynamicResult{
		Points:  revPoints,
		Revenue: &billing.RevenueDynamicMainData{Count: 0, Total: 0, Avg: 0},
		Refund:  &billing.RevenueDynamicMainData{Count: 0, Total: 0, Avg: 0},
	}

	rev := res[0][revenueDynamicFacetFieldRevenue].([]interface{})

	if len(rev) > 0 {
		mRev := rev[0].(map[string]interface{})

		if v, ok := mRev[revenueDynamicFacetFieldCount]; ok {
			rsp.Revenue.Count = v.(int32)
		}

		if v, ok := mRev[revenueDynamicFacetFieldTotal]; ok {
			rsp.Revenue.Total = formatAmount(v.(float64))
		}

		if v, ok := mRev[revenueDynamicFacetFieldAvg]; ok {
			rsp.Revenue.Avg = formatAmount(v.(float64))
		}
	}

	ref := res[0][revenueDynamicFacetFieldRefund].([]interface{})

	if len(ref) > 0 {
		mRef := ref[0].(map[string]interface{})

		if v, ok := mRef[revenueDynamicFacetFieldCount]; ok {
			rsp.Refund.Count = v.(int32)
		}

		if v, ok := mRef[revenueDynamicFacetFieldTotal]; ok {
			rsp.Refund.Total = formatAmount(v.(float64))
		}

		if v, ok := mRef[revenueDynamicFacetFieldAvg]; ok {
			rsp.Refund.Avg = formatAmount(v.(float64))
		}
	}
...
func (s *Service) getRevenueDynamic(req *grpc.RevenueDynamicRequest) ([]map[string]interface{}, error) {
	...
	switch req.Period {
	case revenueDynamicRequestPeriodYear:
		delete(mGroup, revenueDynamicRequestPeriodHour)
		delete(mGroup, revenueDynamicRequestPeriodDay)
		delete(mGroup, revenueDynamicRequestPeriodWeek)
		delete(mGroup, revenueDynamicRequestPeriodMonth)
		break
	case revenueDynamicRequestPeriodMonth:
		delete(mGroup, revenueDynamicRequestPeriodHour)
		delete(mGroup, revenueDynamicRequestPeriodDay)
		delete(mGroup, revenueDynamicRequestPeriodWeek)
		break
	case revenueDynamicRequestPeriodWeek:
		delete(mGroup, revenueDynamicRequestPeriodHour)
		delete(mGroup, revenueDynamicRequestPeriodDay)
		break
	case revenueDynamicRequestPeriodDay:
		delete(mGroup, revenueDynamicRequestPeriodHour)
		break
	}
	...
	q := []bson.M{
		{
			"$project": bson.M{
				revenueDynamicRequestPeriodHour:  bson.M{"$hour": "$created_at"},
				revenueDynamicRequestPeriodDay:   bson.M{"$dayOfMonth": "$created_at"},
				revenueDynamicRequestPeriodWeek:  bson.M{"$week": "$created_at"},
				revenueDynamicRequestPeriodMonth: bson.M{"$month": "$created_at"},
				revenueDynamicRequestPeriodYear:  bson.M{"$year": "$created_at"},
				"project":                        true,
				"status":                         true,
		...
		{
			"$facet": bson.M{
				revenueDynamicFacetFieldRevenue: []bson.M{
					{
						"$match": bson.M{
							"status":     OrderStatusProjectComplete,
							"created_at": bson.M{"$gte": req.From, "$lte": req.To},
							"project.id": bson.M{"$in": req.Project},
						},
					},
					{
						"$group": bson.M{
							revenueDynamicFacetFieldId:    "$project.merchant._id",
							revenueDynamicFacetFieldCount: bson.M{"$sum": 1},
							revenueDynamicFacetFieldTotal: bson.M{"$sum": "$amount_out_merchant_ac"},
							revenueDynamicFacetFieldAvg:   bson.M{"$avg": "$amount_out_merchant_ac"},
						},
					},
				},
				revenueDynamicFacetFieldRefund: []bson.M{
					{
						"$match": bson.M{
							"status":     bson.M{"$in": []int{OrderStatusRefund, OrderStatusChargeBack}},
							"created_at": bson.M{"$gte": req.From, "$lte": req.To},
							"project.id": bson.M{"$in": req.Project},
						},
					},
				...
				revenueDynamicFacetFieldPointsRevenue: []bson.M{
					{
						"$match": bson.M{
							"status":     OrderStatusProjectComplete,
							"created_at": bson.M{"$gte": req.From, "$lte": req.To},
					...
					{
						"$group": bson.M{
							revenueDynamicFacetFieldId:    mGroup,
							revenueDynamicFacetFieldTotal: bson.M{"$sum": "$amount_out_merchant_ac"},
						},
					},
					{"$sort": bson.M{revenueDynamicFacetFieldId: 1}},
				},
				revenueDynamicFacetFieldPointsRefund: []bson.M{
					{
						"$match": bson.M{
							"status":     bson.M{"$in": []int{OrderStatusRefund, OrderStatusChargeBack}},
							"created_at": bson.M{"$gte": req.From, "$lte": req.To},
	...
	err := s.db.Collection(pkg.CollectionOrder).Pipe(q).All(&result)

	return result, err
}

// Get data about accounting payment by accounting period of merchant
func (s *Service) GetAccountingPayment(
	ctx context.Context,
	req *grpc.RevenueDynamicRequest,
	...
	if v, ok := apData["total_success"]; ok {
		...
		if len(sV) > 0 {
			...
			if vv, ok := mV["total"]; ok {
				rsp.SuccessWithCommissions = vv.(float64)
			}
		}
	}

	if v, ok := apData["success"]; ok {
		rsp.SuccessWithoutCommissions = getValueFromAccountingPaymentReport(v)
	}

	if v, ok := apData["refund"]; ok {
		rsp.TotalRefund = getValueFromAccountingPaymentReport(v)
	}

	if v, ok := apData["chargeback"]; ok {
		rsp.TotalChargeback = getValueFromAccountingPaymentReport(v)
	}

	if v, ok := apData["commission"]; ok {
		rsp.TotalCommission = getValueFromAccountingPaymentReport(v)
	}

	return nil
}

func getValueFromAccountingPaymentReport(v interface{}) float64 {
	var fV float64
...
func (s *Service) getAccountingPayment(req *grpc.RevenueDynamicRequest) ([]map[string]interface{}, error) {
	var result []map[string]interface{}
		...
		},
		{
			"$facet": bson.M{
				"total_success": []bson.M{
					{
						"$match": bson.M{
							"status":                       bson.M{"$in": []int{OrderStatusProjectComplete, OrderStatusPaymentSystemComplete, OrderStatusProjectInProgress, OrderStatusProjectPending}},
							"pm_order_close_date":          bson.M{"$gte": req.From, "$lte": req.To},
							"project.merchant.external_id": bson.M{"$eq": req.MerchantIdentifier},
						},
					},
					{
						"$group": bson.M{
...
func getRevenueDynamicPointsKey(pointId map[string]interface{}) *billing.RevenueDynamicPointDate {
	revPointDate := &billing.RevenueDynamicPointDate{
		Year: pointId[revenueDynamicRequestPeriodYear].(int32),
	}

	if val, ok := pointId[revenueDynamicRequestPeriodMonth]; ok {
		revPointDate.Month = val.(int32)
	}

	if val, ok := pointId[revenueDynamicRequestPeriodWeek]; ok {
		revPointDate.Week = val.(int32)
	}

	if val, ok := pointId[revenueDynamicRequestPeriodDay]; ok {
		revPointDate.Day = val.(int32)
	}

	if val, ok := pointId[revenueDynamicRequestPeriodHour]; ok {
		revPointDate.Hour = val.(int32)
	}
